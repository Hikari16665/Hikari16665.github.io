...
歌词可与 MP3 放在同一目录，支持 `歌名-作曲家.lrc` 和 `作曲家 - 歌名.lrc` 两种命名。播放器兼容标准 LRC 时间戳、毫秒时间戳、`offset` 标签，以及网易云导出文件开头的 JSON 制作信息。存在匹配歌词时，播放器中的 `PV` 按钮会启用页面背景歌词可视化。

可选的 `歌名-作曲家.pv.json` 会覆盖通用 PV 主题，可配置 `palette`、`intensity`、`typography`、`spectrum`、`motion` 与歌词 `keywords` 场景词。
